...
- GPIO 33（Trig）/GPIO 32（Echo）：超声波测距结果。扩展板插座上的
  `A0/A1` 是接口名称，ESP32 示例程序实际使用 `Trig=33、Echo=32`。

`c001_motor_diagnostic` 是一次性低速电机诊断程序。它使用 GPIO
16/17 驱动左电机、GPIO 26/27 驱动右电机，上电等待八秒后依次执行
左右轮正反转和双轮前进，完成后保持停车。

`c001_obstacle_stop_diagnostic` 用于架空车轮后的超声波刹停测试。烧录后
电机保持停止，按下并松开 GPIO25 才会低速正转；距离连续三次小于
25 厘米、再次按下按钮或运行满十秒都会锁定停车。障碍移开后不会自动重启。
